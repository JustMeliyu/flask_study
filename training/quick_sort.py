    ...
    if len(origin_list) > 1:
        ...
        for i in range(len(origin_list) - 1, -1, -1):
            if start_index == i + 1:
                ...
                origin_list[0], origin_list[start_index] = origin_list[start_index], origin_list[0]
                return quick_sort2(origin_list[:start_index]) + [origin_list[start_index]] + quick_sort2(
                    origin_list[start_index + 1:])
                # else:
...
def quick_sort4(myList):
    ...
    if len(myList) > 0:
        ...
        # 递归前后半区
        return quick_sort4(myList[:i-1]) + [myList[i]] + quick_sort4(myList[j + 1:])
    return myList
