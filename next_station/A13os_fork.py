...
def daemonize(stdin='/dev/null', stdout='/dev/null', stderr='dev/null'):
    ...
    print "a,current ID is : {0}".format(os.getpid())
    ...
    # 执行第二次fork
    print "b,current ID is : {0}".format(os.getpid())
    try:
        pid = os.fork()
        print pid
        if pid > 0:
        ...
        sys.exit(1)
    print "c,current ID is : {0}".format(os.getpid())
...
# 第一个fork是为了让shell返回，同时让你完成setsid（从你的控制终端移除，这样就不会意外地收到信号）。
# setsid使得这个进程成为“会话领导（session leader）”，即如果这个进程打开任何终端，该终端就会成为此进程的控制终端。
# 我们不需要一个守护进程有任何控制终端，所以我们又fork一次。在第二次fork之后，此进程不再是一个“会话领导”，
# 这样它就能打开任何文件（包括终端）且不会意外地再次获得一个控制终端
